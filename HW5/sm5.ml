   ...
   *)
   ...
   let malloc_with_gc s m e c k =
     if List.length m < mem_limit then
       ...
       let rec rec2reach : record -> (loc list) = fun r -> (
         match r with
         | [] -> []
         | head :: tail -> (snd head) :: (rec2reach tail)
       ) in
 
       let loc2reach : loc -> (loc list) = fun l -> (
         [l]
       ) in
 
       let val2reach : value -> (loc list) = fun value -> (
         match value with
         | L l ->   loc2reach l
         | R r ->   (rec2reach r)
         | _ ->  []
       ...
       let rec cmd2reach : command -> (loc list) = fun cmd -> (
         match cmd with
         | [] -> []
         | head ::tail ->(
           match head with
           | PUSH obj -> (obj2reach obj) @ (cmd2reach tail)
           | JTR (cmd1, cmd2) -> (cmd2reach cmd1) @ (cmd2reach cmd2) @ (cmd2reach tail)
           | _ -> (cmd2reach tail)
         )
       )
       and obj2reach : obj -> (loc list) = fun obj -> (
         match obj with
         | Val v -> val2reach v
         | Fn (str, cmd) -> cmd2reach cmd
         | Id x -> []
       ) in
 
       let rec env2reach : environment -> (loc list) = fun env ->(
         match env with
         | [] -> []
         | (x, evalue) :: tail -> (eval2reach evalue) @ (env2reach tail)
       )
       and eval2reach : evalue -> (loc list) = fun eval -> (
         match eval with
         | Loc l -> loc2reach l
         | Proc (str, cmd, env) -> (cmd2reach cmd) @ (env2reach env)
       ) in
 
       let sval2reach : svalue -> (loc list) = fun sval -> (
         match sval with
         | V v -> val2reach v
         | P (str, cmd, env) -> (cmd2reach cmd) @ (env2reach env)
         | M (str, eval) -> (eval2reach eval)
       ) in
 
       let rec stack2reach : stack -> (loc list) = fun stack -> (
         match stack with
         | [] -> []
         | head ::tail -> (sval2reach head) @ (stack2reach tail) 
       ) in
 
       let rec contin2reach : continuation -> (loc list) = fun con ->(
         match con with
         | [] -> []
         | (cmd, env) :: tail -> (cmd2reach cmd) @ (env2reach env) @ (contin2reach tail)
       ) in
 
       let rec mem2reachTemp : (loc list) -> memory -> (loc list) = fun list mem -> (
         match mem with
         | [] -> list
         | (l, v) :: tail -> (
           if List.mem l list then (val2reach v) @ (mem2reachTemp list tail)
           else (mem2reachTemp list tail)
         )
       ) in
 
       let rec refineList : (loc list) -> (loc list) = fun list -> (
         match list with
         | [] -> []
         | head :: tail -> if List.mem head tail then refineList tail else head :: refineList tail
       ) in
 
       let _ = reachable_locs := (stack2reach s) @ (cmd2reach c) @ (env2reach e) @ (contin2reach k) in
       (* TODO : Add the code that marks the reachable locations.
        * let _ = ... 
        *)
       let rec mem2reach : (loc list) -> memory -> (loc list) = fun list mem -> (
         let list' = mem2reachTemp list mem in
         let list'' = refineList list' in
         if List.length list'' > List.length list then mem2reach list'' mem else list''
       ) in
 
       let _ = reachable_locs := mem2reach !reachable_locs m in
 
       let new_m = List.filter (fun (l, _) -> List.mem (fst l,0) !reachable_locs) m in
