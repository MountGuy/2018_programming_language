...
let rec unify : (typ * typ) -> subst = fun (t1, t2) ->
  let _ = print_endline(typ_to_str t1 ^ " = " ^ typ_to_str t2) in 
  match t1 with
  | TInt ->
  (
    match t2 with
    ...
    | _ -> let _ = print_endline("1") in raise(M.TypeError"oops")
  )
  | TBool ->
  (
    match t2 with
    | TBool -> empty_subst
    | TVar x -> make_subst x t1
    | _ -> let _ = print_endline("2") in raise(M.TypeError"oops")
  )
  | TString ->
  (
    match t2 with
    | TString -> empty_subst
    | TVar x -> make_subst x t1
    | _ -> let _ = print_endline("3") in raise(M.TypeError"oops")
  )
  | TPair (t1f, t1s) ->
  (
    match t2 with
    | TPair (t2f, t2s) ->
      ...
      s @@ s'
    | TVar x -> 
      if invalid (t1, x) then let _ = print_endline("4") in raise(M.TypeError"oops") else make_subst x t1
    | _ -> let _ = print_endline("5") in raise(M.TypeError"oops")
  )
  | TLoc t1l ->
  (
    match t2 with
    ...
    | TVar x -> 
      if invalid (t1, x) then let _ = print_endline("6") in raise(M.TypeError"oops") else make_subst x t1
    | _ -> let _ = print_endline("7") in raise(M.TypeError"oops")
  )
  | TFun (t1i, t1o) ->
  (
    match t2 with
    | TFun (t2i, t2o) ->
      ...
      s @@ s'
    | TVar x ->
      if invalid (t1, x) then let _ = print_endline("8") in raise(M.TypeError"oops") else make_subst x t1
    | _ -> let _ = print_endline("9") in raise(M.TypeError"oops")
  )
  | TVar x1 ->
  (
    match t2 with
    | TVar x2 -> if x1 = x2 then empty_subst else (make_subst x1 t2) @@ (make_subst x2 t1)
    | _ ->
      if invalid(t2, x1) then 
      let _ = print_endline("10") in raise(M.TypeError"oops") else make_subst x1 t2
  )
  (*
    match t2 with
...
let check : M.exp -> M.typ = fun exp ->
  ...
  let rec typ2mtyp ans =
    ...
    |  _ -> let _ = print_endline("11") in raise(M.TypeError"asdf") in
