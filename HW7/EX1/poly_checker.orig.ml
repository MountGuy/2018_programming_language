...
let rec typ_to_str t = 
 ...
 let make_subst : var -> typ -> subst = fun x t ->
   let _ = print_endline(x ^ " = " ^ typ_to_str t ^ "added") in
   let rec subs t' =
     match t' with
     | TVar x'
     | TVarIBSL x'
...
let rec online : typ_env -> M.exp -> (subst* typ) = fun tyenv exp ->
  ...
  | M.VAR x ->
    ...
    (
      match s_sig with
      | SimpleTyp t -> let _ = print_endline("["^x ^ " = "^typ_to_str t) in (empty_subst, t)
      | GenTyp (_, t) -> let _ = print_endline("["^x ^ " = "^typ_to_str t) in (empty_subst, t)
    )
  | M.FN (x, e) ->
  ...
  (
    ...
    | M.REC (f, x, e1) ->
      ...
      let tyenv' = subst_env (s1 @@ s2) tyenv in 
